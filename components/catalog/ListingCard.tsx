...
const ListingCard: React.FC<ListingCardProps> = ({ listing }) => {
  const mainImage =
    listing.media_files?.find((f) => f.is_main) ?? listing.media_files?.[0];

  const hasDocuments =
    listing.media_files?.some((f) => f.type === 'document') ?? false;
  ...
  const getListingUrl = () => {
    if ('area' in listing) return `/catalog/land-plots/${listing.slug}`;
    if ('capacity' in listing) return `/catalog/helicopters/${listing.slug}`;
    return `/catalog/properties/${listing.slug}`;
  };

  return (
    <Link href={getListingUrl()}>
      <div className="bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer group h-full flex flex-col">
        {/* Image */}
        <div className="h-64 bg-gray-200 relative overflow-hidden flex-shrink-0">
          {mainImage ? (
            <img
              src={mainImage.file_url ?? mainImage.url}
              alt={listing.title}
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          <div className="flex items-center justify-between mb-2 flex-shrink-0">
            <div className="flex items-center gap-2 text-gray-600 flex-1 min-w-0">
              ...
              <span className="text-sm truncate">{getFullAddress()}</span>
            </div>
          ...
          <div className="mt-auto flex-shrink-0 flex items-center justify-between gap-3">
            ...
            <div className="flex-shrink-0">
              {(listing as { plot_status_display?: string }).plot_status_display === 'Продан' ? (
